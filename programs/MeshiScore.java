...
public class MeshiScore extends MeshiProgram implements KeyWords {
    ...
    public static final String NAME = "MeshiScore";
    ...
    //      Secondary structure;
	    ...
	    minimizeCassette(lbfgs, minimizationEnergy, (TetherEnergy) tetherAllCreator.term());
            ...
            MeshiInfo infoList = minimizationEnergy.energyInfo();
            for (Score scoreFunction : scoreFunctions) {
		    ...
		    if (scoreFunction.toString().compareTo("MESHI.SERVER")==0 && score.type.tag.compareTo("weightedMedianScore")==0)
	                    System.out.println( "MESHI"+"\t"+score.getValue()+"\t"+inFileName);
 ...
 //           mcm(optimizationScore, selectionScore, energy, "MCM\"  step=\""+i+"\" score=\""+score.score()+"\" result=\""+mcmStepResult+"\"  lastSuccess=\""+mcmStepResult.lastSuccess());
    ...
    private static void init(String[] argv) {
        //                 0            1                           
        String[] keys = {"commands", "inFileName"};
	...
	/*	
	if (commands.keyExists("auxPath")) {
		String dsspFileName = inFileName.replace(".pdb",".dssp").substring(inFileName.lastIndexOf(File.separator)+1); 
		dsspFile   = commands.firstWord("auxPath").secondWord()+dsspFileName;
	}
	else dsspFile = "NONE"; 
	*/
	dsspFile   = inFileName.replace(".pdb",".dssp");
