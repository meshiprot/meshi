...
public class AtomList extends ArrayList<Atom> {
    ...
    public AtomList(PdbLineIterator lineIterator) {
        PdbATOMLine line;
...
/*        if (!(file instanceof PdbReader))
        ...
        while (lineIterator.hasNext() & contRead) {
            line = (PdbATOMLine) lineIterator.next();
            //if (line.isAModel() & (line.getModel() != 1))      ToDo Take care of NMR multiple models
            //contRead = false;
            //if (lineFilter.accept(line)) { //No Need, the filter is in the iterator, only ATOM lines gets through.
    ...
    /**
     * An atom list with correcting modified residues.
     */
    public AtomList(String fileName, PdbHeader pdbHeader, Filter lineFilter) {
        this(new PdbReader(fileName,pdbHeader), lineFilter);
    }
