...
import meshi.energy.solvation.SolvateParametersList;
...
public class HydrogenBondDahiyatList extends AbstractHydrogenBondList {
    ...
    /**
     * This array stores pointers to the base getAtoms of every non-hydrogen polar atom in the protein.
     * The indexation is through the atom number (field)
     */
    protected Atom[] baseAtom;
    /**
     * Since currently the hydrogen bond list is only used by the solvation energy, we use the parameters
    ...
    protected void buildSpecificStructures() {
        ...
        for (int iAtom = 0; iAtom < atomList.size(); iAtom++) {
            atom1 = atomList.atomAt(iAtom);
            for (int jBondedAtom = 0; jBondedAtom < atom1.bonded().size(); jBondedAtom++) {
                atom2 = atom1.bonded().atomAt(jBondedAtom);
                // Treating OXYGEN toms.
    ...
    protected AbstractHydrogenBond createHBfromPolars(Atom atom1, Atom atom2) {
        if (atom1.nowhere()) return null;
        if (atom2.nowhere()) return null;
        if (baseAtom[lut[atom1.number()]] == null) return null;
        if (baseAtom[lut[atom2.number()]] == null) return null;


        if (baseAtom[lut[atom1.number()]].nowhere()) {
            return null;
        }
        if (baseAtom[lut[atom2.number()]].nowhere()) return null;

        if ((atom1.type() != AtomType.CSG) || (atom2.type() != AtomType.CSG))   // CSD is donor in normal hydrogen bonds, but here we also consider disulphide a H-bond
            if (!((atom1.type().HbDonor() & atom2.type().HbAcceptor()) ||
                (atom2.type().HbDonor() & atom1.type().HbAcceptor()))) return null;
        ...
        if (end < 0.1)  // With the current paramters for the solvation energy: end=0 means that hydrogen bonding is not relevent to this pair.
            return null;
