...
public class SecondaryStructureAlphabetFeatures_plusNoise extends AbstractEnergy {
    ...
    public SecondaryStructureAlphabetFeatures_plusNoise(Protein model, MeshiSequence[] sequenceWithPredictions) {
	...
	double num = rnd.nextDouble();
	    ...
	    boolean b = false;
            if (residue != null && prediction != null) {
                if (residue.getLocalStructure().equals(prediction,localStructureAlphabetType)) {
			if (prediction.getDSSP7Reduction() == DsspLocalStructureLetter.HELIX ) {
				if ( num <= 0.878 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.THREE10HELIX ) {
                                if ( num <= 0.53 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.PIHELIX ) {
                                if ( num < 0.0 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.BEND ) {
                                if ( num <= 0.537 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.TURN ) {
                                if ( num <= 0.589 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.COIL ) {
                                if ( num <= 0.606 ) b = true;
                        } else if(prediction.getDSSP7Reduction() == DsspLocalStructureLetter.SHEET ) {
				if (localStructureAlphabetType == LocalStructureAlphabetType.STR2) {
					if ( num <= 0.4 ) b = true;
				} else if ( num <= 0.803 ) b = true;
                        } 
			
		}
		if ( b == true || num <= 0.1 ) sumObserved++;
                sumPredicted++;
